...
import { ItemClassField } from "./types";
...
export default function ItemsClass() {
  const { useSnackBar } = useContext(GlobalContext);
  const { account } = useAccountStore();
  const { data, refetch } = getItemClassQuery(account);
  ...
  const form = useForm<ItemClassField>();
  ...
  const formState = () => {
    const formReset = form.reset({
      itmclacde: isDocnumChecked ? docNum : "",
      itmcladsc: "",
    });
    return formReset;
  };
  ...
  const onSubmitAdd = async () => {
    try {
      ...
      const res = await addItemClassQuery(account, formData, isDocnumChecked);
  ...
  const onSubmitEdit = async (row: any) => {
    try {
      const res = await editItemClassQuery(account, primaryId, row);
      if (res.status === "Success") {
        useSnackBar(res.payload?.msg, { variant: "success" });
        setEditPageEnable(false);
        setMainPageEnable(true);
      ...
      const errorMessage = e.response.data.payload.msg;
      const error = errorMessage ? errorMessage : e;
      useSnackBar(`Item Classification Update: ${error}`, { variant: "error" });
    }
    refetch();
  };
  ...
  const onSubmitDelete = async (row: any) => {
    try {
      const res = await deleteItemClassQuery(primaryId, account);
      if (res.status === "Success") {
        useSnackBar(res.payload?.msg, { variant: "success" });
        setDialogOpen(false);
      } else {
